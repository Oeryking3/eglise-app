...
const CONTRIBUTION_TYPES = [
  { key: 'offrande', label: 'Offrandes', description: 'Les offrandes soutiennent les besoins et les actions de l’église.', presets: [1000, 2000, 5000, 10000] },
  { key: 'offrande_journaliere', label: 'Offrandes journalières', description: 'Les offrandes journalières commencent à 1 000 F et sont réglées chaque jour.', presets: [1000, 5000, 10000, 500000] },
] as const;

type ContributionType = (typeof CONTRIBUTION_TYPES)[number]['key'];

export default function OffrandesScreen() {
  const params = useLocalSearchParams<{ payment_id?: string }>();
  const [contributionType, setContributionType] = useState<ContributionType>('offrande');
  const [amount, setAmount] = useState('');
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);
  const [loading, setLoading] = useState(false);
  const [checkingReturn, setCheckingReturn] = useState(Platform.OS === 'web' && !!params.payment_id);
  const selectedContribution = CONTRIBUTION_TYPES.find((item) => item.key === contributionType) ?? CONTRIBUTION_TYPES[0];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (check.data.data.statut === 'reussi') setSuccess(true);
        else setError(check.data.data.statut === 'echoue' ? 'Le paiement a échoué. Réessaie.' : 'Le paiement n’est pas encore confirmé.');
      } catch (e) {
  ...
  const onSubmit = async () => {
    ...
    const minimumAmount = contributionType === 'offrande_journaliere' ? 1000 : 500;
    if (!Number.isInteger(contributionAmount) || contributionAmount < minimumAmount) {
      setError(`Le montant minimum est de ${minimumAmount.toLocaleString('fr-FR')} FCFA.`);
      return;
    }
    ...
    setLoading(true);
    try {
      const returnUrl = Platform.OS === 'web' && typeof window !== 'undefined' ? `${window.location.origin}/offrandes` : Linking.createURL('offrandes-retour');
      const { data } = await api.post<{ payment: { id: number }; payment_url: string }>('/don', {
        montant: contributionAmount,
  ...
  if (checkingReturn) return <View style={styles.center}><ActivityIndicator size="large" color={colors.orange} /></View>;
  if (success) {
    return <View style={styles.center}>
      <View style={styles.successIcon}><Feather name="heart" size={30} color={colors.orange} /></View>
      <Text style={styles.successTitle}>Merci pour ton offrande</Text>
  ...
  return <View style={styles.page}>
    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={styles.types}>
        {CONTRIBUTION_TYPES.map((item) => <Pressable key={item.key} onPress={() => { setContributionType(item.key); setAmount(''); }} style={[styles.type, contributionType === item.key && styles.typeActive]}>
          <Text style={[styles.typeText, contributionType === item.key && styles.typeTextActive]}>{item.label}</Text>
        </Pressable>)}
      </View>
      <Text style={styles.label}>Choisis un montant</Text>
      <View style={styles.presets}>
        {selectedContribution.presets.map((preset) => <Pressable key={preset} onPress={() => setAmount(String(preset))} style={[styles.preset, amount === String(preset) && styles.presetActive]}>
      ...
      <View style={styles.amountInput}><TextInput value={amount} onChangeText={setAmount} keyboardType="number-pad" placeholder="Montant" placeholderTextColor={colors.textPlaceholder} style={styles.input} /><Text style={styles.currency}>FCFA</Text></View>
      <Text style={styles.hint}>Le paiement sécurisé te permettra de choisir Wave, Orange Money, MTN Money, Moov Money ou carte bancaire. L’offrande journalière correspond à un règlement pour la journée en cours.</Text>
...
const styles = StyleSheet.create({
  ...
  type: { borderColor: colors.orangeBorder, borderRadius: radii.md, borderWidth: 1, paddingHorizontal: spacing.md, paddingVertical: spacing.md },
  typeActive: { backgroundColor: colors.orange, borderColor: colors.orange },
  typeText: { color: colors.orangeDark, fontSize: 14, fontWeight: '700' },
