...
<title>Nouveau rappel - Église Ambassade des Miracles</title>
    ...
    <div class="card">
      <form method="POST" action="{{ route('agenda.store') }}">
        ...
        <div class="form-row">
            <div class="form-group">
                <label>Date</label>
                <input type="date" name="date_rappel" value="{{ old('date_rappel') }}" required>
            </div>
            <div class="form-group">
                <label>Heure (facultatif)</label>
                <input type="time" name="heure_rappel" value="{{ old('heure_rappel') }}">
            </div>
        </div>

        <button type="submit" class="submit-btn">Ajouter le rappel</button>
      </form>
    </div>
  </main>
